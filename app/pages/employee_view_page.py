...
class EmployeeViewPage(tk.Frame):
    def __init__(self, master: tk.Misc, on_save: Callable[[int, dict], None], on_delete: Callable[[int], None], on_cancel: Callable[[], None]) -> None:
        ...
        for idx, (key, label) in enumerate(FIELDS):
            ...
            # Edit widget
            if key == "affiliation":
                field = ttk.Combobox(self.form_container, textvariable=var, state="readonly", values=AFFILIATION_UI_VALUES, font=(FONT_FAMILY, 10))
            else:
                field = ctk.CTkEntry(
                    self.form_container,
                    textvariable=var,
        ...
        self.date_info.pack_forget() # Show only in edit mode
